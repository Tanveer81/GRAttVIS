...
class VITA(nn.Module):
    ...
    @torch.jit.unused
    def _set_aux_loss(
        self, outputs_cls, outputs_mask_embed, outputs_cq_embed, outputs_fq_embed, pred_prop_soft
    ):
        return [{"pred_logits": a, "pred_mask_embed": b, "pred_cq_embed": c, "pred_fq_embed": outputs_fq_embed,
                 "pred_prop_soft": d}
                for a, b, c, d  in zip(outputs_cls[:-1], outputs_mask_embed[:-1], outputs_cq_embed[:-1], pred_prop_soft[:-1])]
